...
class Delivery extends Component {
  ...
  delive = async () => {
    ...
    await contract.methods.delivery(_id,_review).send({ from: accounts[0]});
  ...
  render() {
    ...
    return (
      <div className="App">
        <h1>Agriculture Food Supply Chain Management Using Blockchain</h1><br />
        <p><b>Delivery Page</b></p>
        <div className="buy">
          <table>
          <tr><td>Enter Product ID : </td><td></td><td><input type="text" id = "pid" placeholder="Enter Product ID"></input></td></tr><br></br>
          <tr><td>Give Your Review : </td><td></td><td><input type="text" id = "review" placeholder="Give Your Review"></input></td></tr><br></br>
          </table>
        <button type="submit" className="btn" onClick={this.delive}>Confirm Delivery & Display Details</button>
        </div>
        <div>
        ...
        <div>
            <button type="submit" className="btn1">Resell</button>
        </div>
     </div>
    );
  }
